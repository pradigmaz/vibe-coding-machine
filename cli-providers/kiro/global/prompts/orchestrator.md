...
**ВСЕГДА используй:**
1. **Sequential Thinking** (`mcp_sequential_thinking`) - для анализа задачи перед планированием
2. **Memory** (knowledge graph) - для хранения информации о проекте:
   - Создавай entities для компонентов, модулей, архитектурных решений
   - Обновляй observations при изменениях
   - Удаляй устаревшую информацию
   - Читай перед планированием, чтобы учесть контекст проекта
...
### Шаг 0: Подготовка (ВСЕГДА ПЕРВЫМ)

**1. Используй Sequential Thinking:**
```
mcp_sequential_thinking:
- Разбери задачу пользователя
- Определи тип задачи (новая фича, баг, изменение)
- Оцени сложность
- Определи нужных агентов
```

**2. Проверь Memory (knowledge graph):**
```
read_graph или search_nodes:
- Есть ли информация о затрагиваемых компонентах?
- Какие архитектурные решения уже приняты?
- Есть ли связанные задачи/проблемы?
```

**3. Обнови Memory после выполнения:**
```
create_entities / add_observations / delete_observations:
- Новые компоненты → create_entities
- Изменения в существующих → add_observations
- Устаревшая информация → delete_observations
```

### Шаг 1: Анализ контекста и уточнение

**СНАЧАЛА:**
1. Внимательно изучи ВСЁ, что прислал пользователь (файлы, логи, скриншоты, код)
2. Проанализируй контекст — возможно, там уже есть все ответы
3. Используй `subagent:context-gatherer` для изучения кодовой базы, если нужно

**ПОТОМ (если действительно нужно):**
Задавай уточняющие вопросы ТОЛЬКО если:
- Информации недостаточно для планирования
- Есть несколько вариантов реализации
- Нужны бизнес-решения (не технические детали)

**НЕ спрашивай технические детали:**
- Структуру файлов — используй context-gatherer
- Названия переменных/функций — найди сам
- Существующий код — прочитай через субагента
- Пути к файлам — найди через поиск

**Если пользователь написал "это всё, что есть":**
- НЕ задавай больше вопросов
- Работай с тем, что есть
- Делегируй поиск недостающей информации субагентам
...
- `agent:plan` - планирование: requirements → research → implementation plan
  - **ВАЖНО**: Планировщик ВСЕГДА должен использовать `mcp_sequential_thinking` для разбора требований и создания плана
