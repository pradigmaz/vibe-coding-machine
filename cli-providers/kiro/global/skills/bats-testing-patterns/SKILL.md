...
Тестирование shell скриптов с использованием Bats (Bash Automated Testing System). Применяется при написании тестов для shell скриптов, CI/CD pipelines, или TDD разработке shell утилит.

## Когда использовать

- Написание unit тестов для shell скриптов
- Реализация TDD (Test-Driven Development) для скриптов
- Настройка автоматического тестирования в CI/CD
- Тестирование edge cases и error conditions
- Валидация поведения в разных shell окружениях
- Создание maintainable test suites
- Тестирование нескольких shell диалектов (bash, sh, dash)

## Что такое Bats?

Bats (Bash Automated Testing System) - TAP (Test Anything Protocol) совместимый фреймворк для тестирования shell скриптов:
- Простой, естественный синтаксис тестов
- TAP output формат совместимый с CI системами
- Поддержка fixtures и setup/teardown
- Assertion helpers
...
1. **Test one thing per test** - Один тест = одна проверка
2. **Use descriptive test names** - Понятные имена тестов
3. **Clean up after tests** - Всегда удаляй временные файлы в teardown
4. **Test both success and failure paths** - Тестируй и успех, и ошибки
5. **Mock external dependencies** - Изолируй unit под тестом
6. **Use fixtures for complex data** - Используй fixtures для сложных данных
7. **Run tests in CI/CD** - Запускай тесты в CI/CD
8. **Test across shell dialects** - Проверяй портабельность
9. **Keep tests fast** - Запускай параллельно когда возможно
10. **Document complex test setup** - Документируй сложные паттерны

## Checklist для review

Перед отправкой тестов:
- [ ] Каждый тест проверяет одну вещь
- [ ] Описательные имена тестов (@test "Function returns 0 on success")
- [ ] setup() и teardown() используются правильно
- [ ] Временные файлы удаляются в teardown
- [ ] Тестируются и success, и failure пути
- [ ] Внешние зависимости замокированы
- [ ] Используются fixtures для сложных данных
- [ ] Тесты запускаются параллельно где возможно
- [ ] Проверена совместимость с разными shell (bash, sh, dash)
...
```bash
# BAD: test2 зависит от test1
@test "Create file" {
    echo "data" > /tmp/shared.txt
}

@test "Read file" {
    cat /tmp/shared.txt  # Зависит от предыдущего теста!
}
```
...
```bash
setup() {
    echo "data" > "$TMPDIR/shared.txt"
}

@test "Read file" {
    cat "$TMPDIR/shared.txt"
}
```

❌ **Не удаляются временные файлы:**
```bash
@test "Process file" {
    echo "data" > /tmp/test.txt  # Не удаляется!
    run my_function /tmp/test.txt
}
```
...
@test "Function creates output file" {
    my_function arg1
    [ -f "$TMPDIR/output.txt" ]
}
```
...
```bash
@test "Test" {
    run my_function /tmp/file.txt  # Может конфликтовать!
}
```
...
@test "Test" {
    run my_function "$TEST_DIR/file.txt"
}
```
