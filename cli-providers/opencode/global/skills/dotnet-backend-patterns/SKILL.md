...
Паттерны разработки C#/.NET backend для построения надежных API, MCP серверов и enterprise приложений. Покрывает async/await, dependency injection, Entity Framework Core, Dapper, конфигурацию, кэширование и тестирование с xUnit. Применяется при разработке .NET backends, review C# кода, проектировании API архитектур.

## Когда использовать

- Разработка новых .NET Web APIs или MCP серверов
- Review C# кода на качество и производительность
- Проектирование сервисной архитектуры с dependency injection
- Реализация стратегий кэширования с Redis
- Написание unit и integration тестов
- Оптимизация доступа к БД с EF Core или Dapper
- Конфигурирование приложений с IOptions pattern
- Обработка ошибок и реализация resilience patterns
- Выбор между EF Core и Dapper для конкретных задач
- Настройка multi-level кэширования
...
## CLI Commands для .NET Development

### Создание проекта

```bash
# Создать Web API проект
...
# Publish с self-contained runtime
...
# Backup базы данных
dotnet ef dbcontext script -o backup.sql
```

### Code Quality Tools

```bash
# Установить code analyzers
dotnet add package StyleCop.Analyzers
dotnet add package SonarAnalyzer.CSharp

# Запустить code analysis
...
```bash
# Установить dotnet-trace
dotnet tool install --global dotnet-trace

# Собрать trace
dotnet-trace collect --process-id <PID>

# Установить dotnet-counters
...
2. **Dependency Injection**: Constructor injection для всех зависимостей
3. **IOptions Pattern**: Типизированная конфигурация
4. **Result Pattern**: Возвращай Result<T> вместо exceptions для бизнес-логики
5. **Repository Pattern**: Абстракция доступа к данным

### Async/Await
6. **Async All the Way**: Не блокируй с .Result или .Wait()
7. **CancellationToken**: Передавай во все async методы
8. **ConfigureAwait(false)**: В библиотеках для избежания deadlocks
9. **ValueTask**: Для hot paths с кэшированием
10. **No async void**: Только для event handlers

### Database
11. **AsNoTracking()**: Для read-only запросов
12. **Dapper для Reads**: Performance-critical queries
13. **EF Core для Writes**: Complex domain models
14. **Projections**: Select только нужные колонки
15. **Indexes**: Добавляй для частых фильтров

### Caching
16. **Multi-Level Cache**: Memory + Redis
17. **Cache Invalidation**: Стратегия инвалидации
18. **Stale-While-Revalidate**: Для лучшего UX
19. **Cache Keys**: Понятные и структурированные
20. **TTL**: Разумные time-to-live значения

### Testing
21. **Unit Tests**: Для бизнес-логики с Moq
22. **Integration Tests**: Для API с WebApplicationFactory
23. **Test Naming**: Descriptive test names
24. **AAA Pattern**: Arrange, Act, Assert
25. **Test Data Builders**: Для сложных объектов

## Checklist для review

Перед отправкой .NET кода:
- [ ] Async/await используется правильно (no .Result/.Wait())
- [ ] CancellationToken передается во все async методы
- [ ] Dependency injection через constructor
- [ ] IOptions<T> для конфигурации
- [ ] Result<T> для бизнес-логики (не exceptions)
- [ ] AsNoTracking() для read-only EF запросов
- [ ] Dapper для performance-critical reads
- [ ] DTOs используются (не EF entities в API)
- [ ] Validation на API boundaries (FluentValidation)
- [ ] Error handling с proper logging
- [ ] IHttpClientFactory для HTTP clients
- [ ] Using statements для IDisposable
...
public void AddItem(Item item, List<Item> items = new List<Item>())
{
    items.Add(item);
}
```
...
public void AddItem(Item item, List<Item>? items = null)
{
    items ??= new List<Item>();
    items.Add(item);
}
...
```csharp
// BAD: Tight coupling to database schema
[HttpGet("{id}")]
