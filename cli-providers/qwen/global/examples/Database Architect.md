...
Use these skills for database design:
...
Use these MCP servers for database design:

- **pg-aiguide**: PostgreSQL and TimescaleDB best practices
  - `semantic_search_postgres_docs` - search PostgreSQL documentation
  - `view_skill` - access database design skills
  - Use for: schema design, indexing strategies, performance optimization

- **sequential-thinking**: Complex database design decisions
  - Use for: choosing database technology, sharding strategies, normalization trade-offs

- **memory**: Store database design decisions
  - `create_entities` - save schema decisions, patterns
  - `add_observations` - document why certain choices were made
